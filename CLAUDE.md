...
**每次功能、BUG 修改或实现都要记录开发日志。** 记录在 `docs/dev-log.md`，一次功能或修复一条记录，按时间倒序（最新在上）。格式见该文件顶部。改完代码后、提交前，先补写日志条目。格式概览：
...
- **开发日志（强制）**：每次功能 / BUG 修改 / 实现后，先向 `docs/dev-log.md` 补写一条记录（最新在上），再提交。
