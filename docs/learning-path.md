...
### 第 5 章 宿主 → 客户端：长轮询（平台历史能力）

> 长轮询是 dsh 平台能力（见能力清单 [04 双向通信通道](plugin-capability-catalog.md#四双向通信通道)），hello-plugin 早期版本曾用它做宿主事件推送。当前实现已改为「普通 RPC 拉取外部数据」（Jira 待办），本插件不再包含长轮询代码，但机制仍值得理解。
...
- **动手做**：参照能力清单，在自己的插件里实现一个 `events/poll` 端点 + 客户端循环（当前 hello-plugin 的 Jira 待办是请求-响应，非长轮询）。
- **产出**：理解为什么第三方自定义事件不适合标准转发；能解释广播语义与超时/abort 兜底。
